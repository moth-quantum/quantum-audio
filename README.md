...
<img width="600" alt="QSM Example" src="https://drive.usercontent.google.com/download?id=1pcJw_8DWezWOpMKQq84vgwdJROG4ZfiX&export=view&authuser=0"><br>
</div>
...
A Scheme's ```decode()``` method uses local [_AerSimulator_](https://github.com/Qiskit/qiskit-aer) as the default backend. Internally, the function calls `utils.execute()` method that performs ```backend.run()```. Any such backend object compatible with Qiskit can be passed to the ```backend=``` parameter of the `decode()` function. To configure this further, please refer to custom [execute functions](#custom_functions).
