...
def test_convert(sqpam, input_audio, num_index_qubits):
    ...
    assert converted_data.tolist() == np.arcsin(np.sqrt((data+1)/2)).tolist()
...
def test_encode(sqpam, input_audio, prepared_circuit, num_samples, converted_data):
...
@pytest.fixture
def counts():
    return Counts({'110 0': 50, '011 1': 114, '001 1': 51, '011 0': 8, '100 0': 106, '001 0': 76, '000 0': 57, '101 0': 114, '111 0': 67, '100 1': 13, '010 1': 100, '010 0': 44, '000 1': 58, '111 1': 60, '110 1': 82})
...
def result(counts, shots, num_samples):
    return Result.from_dict({'results': [{'shots': shots, 'success': True, 'data': {'counts': counts}, 'header': {'qreg_sizes': [['amplitude', 1], ['time', 3]],'metadata': {'num_samples': num_samples}}}], 'backend_name': 'qasm_simulator', 'backend_version': '0.14.2', 'qobj_id': 'b1b1b1b1-b1b1-b1b1-b1b1-b1b1b1b1b1b1', 'job_id': 'b1b1b1b1-b1b1-b1b1-b1b1-b1b1b1b1b1b1', 'success': True})
