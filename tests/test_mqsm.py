...
test_inputs = [
    ...
    np.array([[0., -0.25, 0.5 , 0.75,  -0.75  ,  -1.,  0.25],[0., 0.25, -0.5 , -0.75,   0.75  ,   0.75, -0.25]])
    ]

test_prepared_data = [
    np.array([0., 0., -0.25, 0., 0.5, 0., 0.75, 0., -0.75, 0., -1., 0., 0.25, 0., 0., 0.]),
    np.array([0., 0., -0.25, 0.25, 0.5, -0.5, 0.75, -0.75, -0.75, 0.75, -1., 0.75, 0.25, -0.25, 0., 0.])
    ]
...
def test_prepare_data(mqsm, input_audio, num_index_qubits, num_channels_qubits, prepared_data):
    data = mqsm.prepare_data(input_audio, num_index_qubits, num_channels_qubits)
...
def test_convert(mqsm, prepared_input, num_index_qubits, qubit_depth):
    ...
    assert converted_data.tolist() == (prepared_input*float(2**(qubit_depth-1))).tolist()
...
def test_initialize_circuit(mqsm, num_index_qubits, num_channels_qubits, num_value_qubits):
    circuit = mqsm.initialize_circuit(num_index_qubits, num_channels_qubits, num_value_qubits)
    assert circuit != None
    assert type(circuit) == QuantumCircuit

@pytest.fixture
def circuit(mqsm, num_index_qubits, num_channels_qubits, num_value_qubits):
    return mqsm.initialize_circuit(num_index_qubits, num_channels_qubits, num_value_qubits)
...
def test_circuit_registers(mqsm, prepared_circuit, num_index_qubits, num_value_qubits, num_channels_qubits):
...
test_counts = [
        {'001 1 000': 290, '011 1 000': 311, '110 1 000': 315, '000 0 000': 306, '001 0 111': 317, '010 0 010': 313, '101 0 100': 334, '111 0 000': 278, '110 0 001': 318, '111 1 000': 314, '100 1 000': 350, '000 1 000': 330, '011 0 011': 319, '010 1 000': 315, '100 0 101': 305, '101 1 000': 285},    {'000 0 000': 241, '111 1 000': 257, '001 0 111': 249, '010 0 010': 253, '110 1 111': 236, '101 1 011': 255, '001 1 001': 253, '100 1 011': 258, '101 0 100': 266, '010 1 110': 281, '011 0 011': 252, '000 1 000': 247, '111 0 000': 222, '110 0 001': 237, '011 1 101': 265, '100 0 101': 228}
]
@pytest.fixture
def counts(request):
...
@pytest.mark.parametrize('counts, exp_components', parameters, indirect=['counts'])
...
def test_reconstruct_data(mqsm, counts, num_components, prepared_data, qubit_depth):
...
def test_decode_result(mqsm, counts, shots, num_samples, num_channels, input_audio, qubit_depth):
...
def test_decode(mqsm, encoded_circuit, shots, counts, num_samples, num_channels, qubit_depth):
    result = get_result(counts, shots, num_samples, num_channels)
