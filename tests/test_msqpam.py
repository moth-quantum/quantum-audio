...
def test_msqpam_fixed_attributes(msqpam):
    assert msqpam.name == 'Multi-channel Single-Qubit Probability Amplitude Modulation'
...
test_prepared_data = [
    np.array([0., 0., -0.25, 0., 0.5, 0., 0.75, 0., -0.75, 0., -1., 0., 0.25, 0., 0., 0.]),
    np.array([0., 0., -0.25, 0.25, 0.5, -0.5, 0.75, -0.75, -0.75, 0.75, -1., 1., 0.25, -0.25, 0., 0.])
    ]
...
def test_prepare_data(msqpam, input_audio, num_index_qubits, num_channels_qubits, prepared_data):
    data = msqpam.prepare_data(input_audio, num_index_qubits, num_channels_qubits)
...
def test_convert(msqpam, prepared_input, num_index_qubits):
    ...
    assert converted_data.tolist() == np.arcsin(np.sqrt((prepared_input+1)/2)).tolist()

@pytest.fixture
def converted_data(msqpam, request):
    return msqpam.convert(request.param)#prepared_data

@pytest.mark.parametrize('converted_data', tuple(test_prepared_data), indirect=True)
def test_format(msqpam, converted_data):
    print(f'converted data: {converted_data}')

def test_initialize_circuit(msqpam, num_index_qubits, num_channels_qubits, num_value_qubits):
    circuit = msqpam.initialize_circuit(num_index_qubits, num_channels_qubits, num_value_qubits)
    assert circuit != None
    assert type(circuit) == QuantumCircuit

@pytest.fixture
def circuit(msqpam, num_index_qubits, num_channels_qubits, num_value_qubits):
    return msqpam.initialize_circuit(num_index_qubits, num_channels_qubits, num_value_qubits)
...
@pytest.mark.parametrize('input_audio, prepared_circuit', test_inputs_prepared, indirect=['prepared_circuit'])
...
@pytest.mark.parametrize('encoded_circuit, num_channels', list(zip(test_inputs, test_num_channels)), indirect=True)
...
test_counts = [
    {'001 0 1': 122, '010 0 1': 238, '110 1 0': 172, '100 0 1': 46, '100 0 0': 269, '110 1 1': 145, '111 1 1': 156, '110 0 0': 118, '011 1 0': 138, '100 1 1': 147, '100 1 0': 161, '111 0 0': 174, '000 0 1': 147, '001 1 0': 164, '011 0 1': 245, '010 1 1': 156, '101 1 1': 146, '101 0 0': 327, '111 0 1': 145, '111 1 0': 162, '000 1 0': 150, '011 1 1': 173, '101 1 0': 144, '001 1 1': 140, '000 1 1': 164, '010 1 0': 173, '001 0 0': 204, '110 0 1': 199, '000 0 0': 148, '010 0 0': 84, '011 0 0': 43},
    {'110 0 0': 128, '100 0 0': 272, '111 0 0': 167, '100 1 0': 42, '001 0 0': 189, '011 1 0': 280, '001 0 1': 122, '110 1 0': 205, '010 0 1': 250, '100 0 1': 36, '100 1 1': 270, '001 1 1': 175, '000 1 1': 144, '010 1 1': 93, '101 1 1': 314, '101 0 0': 310, '110 1 1': 131, '111 1 1': 142, '111 1 0': 147, '111 0 1': 164, '011 0 1': 280, '000 0 0': 142, '110 0 1': 205, '011 0 0': 35, '010 0 0': 85, '000 0 1': 133, '001 1 0': 102, '010 1 0': 233, '000 1 0': 158, '011 1 1': 46}
]
@pytest.fixture
def counts(request):
...
test_components = [
        ([[148., 204.,  84.,  43., 269., 327., 118., 174.],
       [150., 164., 173., 138., 161., 144., 172., 162.]], [[147., 122., 238., 245.,  46.,   0., 199., 145.],
       [164., 140., 156., 173., 147., 146., 145., 156.]]),
        ([[142., 189.,  85.,  35., 272., 310., 128., 167.],
 [158., 102., 233., 280.,  42.,   0., 205., 147.]], [[133., 122., 250., 280.,  36.,   0., 205., 164.],
       [144., 175.,  93.,  46., 270., 314., 131., 142.]])
]

parameters = [(a, *b) for a, b in zip(test_counts, test_components)]
@pytest.mark.parametrize('counts, cos_components, sin_components', parameters, indirect=['counts'])
def test_decode_components(msqpam, counts, num_components, cos_components, sin_components):
...
@pytest.mark.parametrize('counts, prepared_data', list(zip(test_counts, test_prepared_data)), indirect=['counts'])
...
def get_result(counts, shots, num_samples, num_channels):
    return Result.from_dict({'results': [{'shots': shots, 'success': True, 'data': {'counts': counts}, 'header': {'qreg_sizes': [['amplitude', 1], ['channel', 1], ['time', 3]],'metadata': {'num_samples': num_samples, 'num_channels': num_channels}}}], 'backend_name': 'qasm_simulator', 'backend_version': '0.14.2', 'qobj_id': 'b1b1b1b1-b1b1-b1b1-b1b1-b1b1b1b1b1b1', 'job_id': 'b1b1b1b1-b1b1-b1b1-b1b1-b1b1b1b1b1b1', 'success': True})

parameters = list(zip(test_counts, test_num_channels, test_inputs))
@pytest.mark.parametrize('counts, num_channels, input_audio', parameters, indirect=['counts', 'num_channels'])
def test_decode_result(msqpam, counts, shots, num_samples, num_channels, input_audio):
...
parameters = list(zip(test_counts, test_num_channels, test_inputs))
@pytest.mark.parametrize('counts, num_channels, encoded_circuit', parameters, indirect=True)
def test_decode(msqpam, encoded_circuit, shots, counts, num_samples, num_channels):
    result = get_result(counts, shots, num_samples, num_channels)
